static const char* shader_pip360Equirect_vs = R"(
#if STEREOMODE==2
...
void main()
{
    ...
    out_data.direction = normalize(in_position);
...
#if FORMAT_IS_STEREO==1
    out_data.scale_offset = vec4( 1.0, 0.5, 0.0, 0.5*float(eye_index) );
#endif

    //out_data.direction = normalize( (display.mEye[eye_index].mMatrix_Cam * vec4(in_position, 1.0)).xyz );
