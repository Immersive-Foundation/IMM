...
namespace ImmExporter
{
	...
	public:
		...
		enum class IMMKeepAliveType       // subtle/minimal procedural effects on top of animated or static content
		{
			None,
		...
		bool Init(IMMKeepAliveType type);
		void Deinit(void);

		IMMKeepAliveType GetType(void) const;
		const Wiggle *GetDataWiggle(void) const;
		const Blink  *GetDataBlink(void) const;

	private:
		IMMKeepAliveType mType;
